...
Use standard ANSI SQL constructs when available and when convenient.

Examples

* Use `COALESCE()` over `ISNULL()`
* Use `CAST()` over `CONVERT()`
* Use `CASE WHEN THEN ELSE END` over `IIF`

## Timestamps

* Use `SYSUTCDATETIME()` for current timestamps.
* Use `UTC` timezone for all timestamps, only translate to local time for display or date logic.

## SQL Procedure Documentation and Style Guide

This part of the guide provides consistent conventions and best practices for documenting and writing SQL Server stored procedures within the DIRECT Framework.
...
* Provide full `@example` section with:
  * Valid, runnable, example parameter declarations
  * Procedure call
...
* Adhere to, and resolve any and all linter/static code analysis warnings
...
tba, recommendations and guidelines for testing and testing code

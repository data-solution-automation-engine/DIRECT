...
AS 
(      
 SELECT @NUMBER_OF_RUNNING_PROCESSES = (SELECT COUNT(*) FROM [900_Direct_Framework].omd.MODULE_INSTANCE WHERE EXECUTION_STATUS_CODE=''E'')
)
GO
...
BEGIN
	...
	SELECT @NUMBER_OF_QUEUED_PROCESSES = (SELECT COUNT(*) AS NUMBER_OF_QUEUED_PROCESSED FROM [900_Direct_Framework].[omd_processing].[vw_QUEUE_MODULE_PROCESSING])
	...
	BEGIN TRY
		EXEC [900_Direct_Framework].[omd].[RunModule]
			@ModuleCode = @PROCESS_NAME
	END TRY
	BEGIN CATCH
		SET @PROCESS_NAME = SUBSTRING(@PROCESS_NAME,1,LEN(@PROCESS_NAME)-5)
		SET @SQL_STRING = N''UPDATE [900_Direct_Framework].omd.MODULE SET INACTIVE_INDICATOR=''''Y'''' WHERE MODULE_CODE=''''''+@PROCESS_NAME+''''''''

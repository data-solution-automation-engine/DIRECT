...
SET IDENTITY_INSERT dbo.RECORD_SOURCE on;
INSERT INTO dbo.RECORD_SOURCE (RECORD_SOURCE_ID, RECORD_SOURCE_CODE)
SELECT *
FROM (
    VALUES (0, N'Data Warehouse')
    ) AS REFDATA(RECORD_SOURCE_ID, RECORD_SOURCE_CODE)
WHERE NOT EXISTS (
        SELECT NULL
        FROM RECORD_SOURCE
        WHERE RECORD_SOURCE.RECORD_SOURCE_ID = REFDATA.RECORD_SOURCE_ID
        );
SET IDENTITY_INSERT dbo.RECORD_SOURCE OFF;
...
AS
(
 SELECT @NUMBER_OF_RUNNING_PROCESSES = (SELECT COUNT(*) FROM [Direct_Framework].omd.BATCH_INSTANCE WHERE EXECUTION_STATUS_CODE=''Executing'')
)
GO
...
BEGIN
  ...
  SELECT @NUMBER_OF_QUEUED_PROCESSES = (SELECT COUNT(*) AS NUMBER_OF_QUEUED_PROCESSED FROM [Direct_Framework].[omd_processing].[vw_QUEUE_BATCH_PROCESSING])
  ...
  BEGIN TRY
    EXEC [Direct_Framework].[omd].[RunBatch]
      @BatchCode = @PROCESS_NAME, @ModuleInstanceIdColumnName = @ModuleInstanceIdColumnName
  END TRY
  BEGIN CATCH
    SET @PROCESS_NAME = SUBSTRING(@PROCESS_NAME,1,LEN(@PROCESS_NAME)-5)
    SET @SQL_STRING = N''UPDATE [Direct_Framework].omd.BATCH SET ACTIVE_INDICATOR=''''N'''' WHERE BATCH_CODE=''''''+@PROCESS_NAME+''''''''

...
EXEC [Testing_Framework].[ut].[RegisterTest]
  ...
  ,@Name = 'test-017-attempting-to-run-a-disabled-module-from-a-batch'
  ,@TestObject = 'DIRECT'
  /* Test procedure */
	,@Debug = 'Y'
  ,@TestCode = '/*
* test-17-attempting-to-run-a-disabled-module-from-a-batch
* Expected outcomes:
* - Framework prevents running a disabled module from a batch
...
EXEC [Testing_Framework].[ut].[RunTest]
   @TestName = 'test-017-attempting-to-run-a-disabled-module-from-a-batch'
  ,@PlanId = NULL
  ,@Debug = 'Y';

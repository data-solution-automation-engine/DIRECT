...
EXEC [Testing_Framework].[ut].[RegisterTest]
  ...
  ,@Name = 'test-7-module-rollback-test-after-abort-where-previous-instance-failed'
  ,@TestObject = 'DIRECT'
  /* Test procedure */
	,@Debug = 'Y'
  ,@TestCode = '/*
...
EXEC [Testing_Framework].[ut].[RunTest]
   @TestName = 'test-7-module-rollback-test-after-abort-where-previous-instance-failed'
  ,@PlanId = NULL
  ,@Debug = 'Y';

...
EXEC [Testing_Framework].[ut].[RegisterTest]
  ...
  ,@Name = 'test-012-batch-run-with-two-modules-module-failing-the-2nd'
  ,@TestObject = 'DIRECT'
  /* Test procedure */
	,@Debug = 'Y'
  ,@TestCode = '/*
* test-12-batch-run-with-two-modules-module-failing-the-2nd
* Expected outcomes:
* - Batch and Module performs as expected when second module fails
...
EXEC [Testing_Framework].[ut].[RunTest]
   @TestName = 'test-012-batch-run-with-two-modules-module-failing-the-2nd'
  ,@PlanId = NULL
  ,@Debug = 'Y';

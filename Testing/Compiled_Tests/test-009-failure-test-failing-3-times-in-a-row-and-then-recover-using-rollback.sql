...
EXEC [Testing_Framework].[ut].[RegisterTest]
  ...
  ,@Name = 'test-009-failure-test-failing-3-times-in-a-row-and-then-recover-using-rollback'
  ,@TestObject = 'DIRECT'
  /* Test procedure */
	,@Debug = 'Y'
  ,@TestCode = '/*
* test-9-failure-test-failing-3-times-in-a-row-and-then-recover-using-rollback
* Expected outcomes:
* - Module performs proper failures and recoveries
...
EXEC [Testing_Framework].[ut].[RunTest]
   @TestName = 'test-009-failure-test-failing-3-times-in-a-row-and-then-recover-using-rollback'
  ,@PlanId = NULL
  ,@Debug = 'Y';
